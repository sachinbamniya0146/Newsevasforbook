...
import { getSessionAdmin, notifySessionAdmin } from '../utils/sessionAdminManager.js';
...
🔧 *setadmin <session>* - Set session admin
🗑️ *removeadmin <session>* - Remove admin
📱 *listadmins* - Show all session admins
...
async function resendMenu(sock, from, state) {
  ...
  if (step === 'awaiting_book' || step === 'awaiting_pdf_book') {
    await sock.sendMessage(from, { text: CONFIG.WELCOME_MSG });
    return;
  }
...
You can place new order after ${Math.ceil((sixh - diff) / (60 * 60 * 1000))} hours.`;
    ...
    if (!userStates.has(from)) {
      ...
      if (text.toLowerCase() === 'pdf' || text === '3') {
        ...
        userStates.set(from, state);
        if (imgPath && fs.existsSync(imgPath)) {
          await sock.sendMessage(from, { image: { url: imgPath }, caption: CONFIG.WELCOME_MSG });
        } else {
          await sock.sendMessage(from, { text: CONFIG.WELCOME_MSG });
        }
...
╚═══════════════════════════════╝`;
      ...
      try {
        ...
        if (sessionAdmin) {
          await notifySessionAdmin(sock, sessionName, sessionAdmin, fwMsg);
...
BOOK_PDFS && CONFIG.BOOK_PDFS[state.pdfBook] && CONFIG.BOOK_PDFS[state.pdfBook][langSelected] ? CONFIG.BOOK_PDFS[state.pdfBook][langSelected] : '';
...
🏘️ *क्या आपका गांव/शहर का नाम "${state.selectedPostOffice}" है?*

_Is your village/city name "${state.selectedPostOffice}"?_

✅ हां, यही मेरा गांव/शहर है: *1* / "Yes"
...
📝 अगर आपका गांव/शहर list में नहीं है तो *3* भेजें
...
कृपया अपने गांव/शहर का नाम और पूरा पता लिखें।
