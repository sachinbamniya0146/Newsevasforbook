...
const pendingSessionAdminSetup = new Map(); // NEW: Track pending admin setup
...
logger.warn('⚠️ Bulk sender update skipped');
}
}
...
sock.ev.on('connection.update', async (update) => {  
  ...
  // Pairing Code Mode (Enhanced with Session Admin Setup)  
...
Settings > Linked Devices > Link with Phone Number;   await sock.sendMessage(adminJid, { text: pairMessage });   } catch (e) {   logger.warn(⚠️ Could not forward pairing code: ${e.message});   }   } catch (e) {   logger.error(❌ Pairing error: ${e.message}`);
}
}
}
...
// Connection Opened  
  if (connection === 'open') {  
    ...
    // Setup Session Admin if provided  
...
📊 Send "help" for commands;   await sock.sendMessage(adminJid, { text: startupMsg });   global.botStartupNotificationSent = true;   }   } catch (e) {   logger.warn(⚠️ Startup notification failed: ${e.message}`);
}
}
...
stopScheduler(); process.exit(0);

}

else { logger.error('❌ Invalid choice'); setTimeout(menu, 500); } }

// ==================== MAIN ==================== (async () => { console.clear(); console.log('\n┌────────────────────────────────────────────────────┐'); console.log(🚀 ${CONFIG.BOT_NAME || 'Gyan Ganga Seva Bot'}); console.log(📦 Version: ${CONFIG.BOT_VERSION || '5.0.0'}); console.log(⏰ Started: ${getTimestamp()}); console.log('└────────────────────────────────────────────────────┘\n');

logger.info('🔄 Initializing bot...');

const restored = await autoStartAll();

if (restored > 0) { console.log(\n┌────────────────────────────────────────────────────┐); logger.success(✅ Restored ${restored} session(s)); logger.info(👤 Admin: ${CONFIG.ADMIN?.PHONE || 'Not configured'}); console.log(└────────────────────────────────────────────────────┘\n); } else { console.log(\n┌────────────────────────────────────────────────────┐); logger.warn('⚠️ No saved sessions found'); logger.info('📱 Link your first WhatsApp account from menu'); console.log(└────────────────────────────────────────────────────┘\n); }

await delay(1000); menu(); })();

// ==================== ERROR HANDLERS ==================== process.on('SIGINT', () => { console.log(\n\n┌────────────────────────────────────────────────────┐); logger.info('🛑 Bot stopping gracefully...'); console.log(💾 Saved ${sessions.size} session(s)); console.log(📨 Handled ${totalMessagesHandled} messages); console.log(⏰ Stopped: ${getTimestamp()}); console.log(└────────────────────────────────────────────────────┘\n); stopScheduler(); process.exit(0); });

process.on('uncaughtException', (e) => { if (!e.message.includes('Bad MAC') && !e.message.includes('Connection Closed')) { logger.error(❌ Uncaught Exception: ${e.message}); } });

process.on('unhandledRejection', (e) => { if (e && e.message && !e.message.includes('Bad MAC') && !e.message.includes('Connection Closed')) { logger.error(❌ Unhandled Rejection: ${e.message}); } });
