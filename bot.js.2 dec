...
const pendingAdminSetup = new Map(); // Store pending admin setup during pairing
...
async function connect(name, mode, phone = null, adminPhone = null) {
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      // Pairing Code Mode (Enhanced with admin setup)
      if ((mode === 'pair') && !pairingCodeAttempted && !state.creds.registered && phone) {
        if (connection === 'open' || connection === 'connecting') {
          ...
          try {
            ...
            // If admin phone provided, set it up
            if (adminPhone) {
              ...
              if (cleanAdminPhone) {
                ...
                console.log(`📋 Admin will be set after connection`);
              }
            }
...
This admin will receive orders from "${name}" session.`;
              }
...
👤 *Session Admin:*
+${adminSetup.adminPhone}

📋 *Order Forwarding:*
✅ Main Admin (You)
✅ Session Admin (+${adminSetup.adminPhone})
✅ Order Group

All orders from "${name}" will be forwarded to all three destinations.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
...
You are now the admin for this WhatsApp session.

📋 *What this means:*
✅ You will receive all order notifications from this session
✅ Orders are also sent to main admin
...
async function menu() {
  ...
  if (choice === '1') {
    ...
    const phoneRaw = await ask('📞 WhatsApp Phone (919876543210): ');
    ...
    console.log(`  1. Main Admin (${CONFIG.ADMIN.PHONE})`);
    ...
    if (wantAdmin.toLowerCase() === 'y' || wantAdmin.toLowerCase() === 'yes') {
      ...
      if (!adminPhone || adminPhone.length < 10) {
        logger.warn('Invalid admin phone, skipping admin setup');
        adminPhone = null;
      } else {
    ...
    logger.info(`Starting: ${name} (+${phone})${adminPhone ? ` with admin +${adminPhone}` : ''}`);
    ...
    try {
      ...
      if (Object.keys(admins).length === 0) {
        ...
        console.log('    OR: Setup during pairing (Option 1 in menu)');
      } else {
